...
// 🚀 TÚNEL HTTP PERSISTENTE AZURE
const httpAgent = new http.Agent({ keepAlive: true, maxSockets: 100 });
const httpsAgent = new https.Agent({ keepAlive: true, maxSockets: 100 });

// CERO DISK I/O PARA MÁXIMA VELOCIDAD
async function tomarCaptura() { return; }
...
// 🧠 MOTOR DE PRODUCCIÓN DIARIA (Antier y Ayer con precisión de Zona Horaria)
function obtenerFechasDinamicas() {
    const fechas = [];
    const hoyLocal = new Date();
    hoyLocal.setHours(hoyLocal.getHours() - 4); // Ajuste a AST (UTC-4)
...
(async () => {
    // Al correr en Docker, usamos el directorio de trabajo actual
    ...
    log.info(`Días a procesar hoy: Antier y Ayer.\n`);
    ...
    for (const fechaActual of fechasABuscar) {
                            ...
                            for (let temp of tempRows) {
                                for (let fotoObj of temp.fotos) {
                                    promesasSubida.push(async () => {
                                        ...
                                        if (bufferData) {
                                            let link = await subirAAzure(finalImageName, bufferData, rutaCarpetaVirtual);
        ...
        if (masterExcelData.length > 0) {
            try {
                ...
                const blobName = `EXCEL_HISTORICO/${masterFileName}`;
                const blockBlobClient = containerClient.getBlockBlobClient(blobName);
